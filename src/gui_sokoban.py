...
def welcome_frame():
    ...
    tk.Label(
        frame,
        text="To call your solve_weighted_sokoban "
        "solver: Solve -> Plan action sequence\n",
    ).grid(row=4, column=0)
...
def make_cell(cell_type, box_weight=None):
    ...
    if box_weight is not None:
        canvas.create_text(
            25, 25, text=str(box_weight), fill="black", font=("Helvetica 15 bold")
        )
    return canvas
...
def move_player(direction):
    ...
    if (next_x, next_y) in warehouse.boxes:
        if (
            try_move_box(
                (next_x, next_y), (next_x + xy_offset[0], next_y + xy_offset[1])
            )
            is False
        ):
...
def try_move_box(location, next_location):
    ...
    if (next_x, next_y) not in warehouse.walls and (
        next_x,
        next_y,
    ) not in warehouse.boxes:
        # can move the box!
